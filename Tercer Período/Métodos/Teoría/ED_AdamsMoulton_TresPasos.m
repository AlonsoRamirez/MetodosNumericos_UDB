...
disp('Método explicito de Adams-Moulton de tres pasos')
...
for i=3:1:n-1
    w(i+1) = w(i) + (h/24)*(9*subs(f,[t,y],[ts(i+1),vw(i+1)]) + 19*subs(f,[t,y],[ts(i),w(i)]) - 5*subs(f,[t,y],[ts(i-1),w(i-1)]) + subs(f,[t,y],[ts(i-2),w(i-2)]));
end
