clc
clear variables
disp('Método de Broyden para dos ecuaciones')
...
f1=input('Introduzca la funcion fx= '); % log(x1^2+x2^2)-sin(x1*x2)-log(2)-log(pi)
f2=input('Introduzca la funcion fy= ');%exp(x1-x2)+cos(x1*x2)
%Primero se sacan las derivadas y asi se evaluan las constantes
f11=diff(f1,x1);
f12=diff(f1,x2);
f21=diff(f2,x1);
f22=diff(f2,x2);
x1=input('Introduzca el valor de x1= '); %1.7
x2=input('Introduzca el valor de x2= '); %1.7
error=input('Introduzca la precision= '); %1e-5
